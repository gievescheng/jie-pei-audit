...
# Find DocumentsTab start/end (line 290 area, 0-indexed 289)
start = next(i for i,l in enumerate(lines) if '// \u2500\u2500\u2500 DOCUMENTS TAB' in l)
end   = next(i for i,l in enumerate(lines) if i > start and '// ─── TRAINING TAB' in l)
print(f"DocumentsTab: lines {start+1}\u2013{end} (replacing with bulk version)")

new_docs_tab = r"""// ─── DOCUMENTS TAB ───────────────────────────────────────────────────────────
...
new_lines = lines[:start] + [l + "\n" if not l.endswith("\n") else l for l in new_docs_tab.splitlines()] + lines[end:]
...
print("DocumentsTab bulk-upload patch OK")
