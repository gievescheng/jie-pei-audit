          ...
          <tbody>
            {filtered.map((a,i) => (
              <tr key={a.id} style={{ background:i%2===0?"rgba(255,255,255,0.02)":"transparent" }}>
                ...
                <td style={{ padding:"10px 12px" }}><button onClick={()=>setModal(a)} style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:6, color:"#94a3b8", cursor:"pointer", padding:"4px 10px", fontSize:11 }}>詳情</button></td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div>
      ...
      <div style={{ background: "rgba(20,184,166,0.06)", border: "1px solid rgba(20,184,166,0.2)", borderRadius: 12, padding: 16, marginBottom: 20 }}>
        ...
        <div style={{ display:"flex", gap:24, flexWrap:"wrap", fontSize:12, color:"#64748b" }}>
          <span>≥ 0.5μm粒子：≤ 1,000 個/立方尺尺</span>
          <span>≥ 5μm粒子：≤ 35 個/立方尺尺</span>
      ...
      <div style={{ overflowX:"auto" }}>
        <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
          <thead><tr>{["編號","日期","地點","≥ 0.5μm","≥ 1μm","≥ 5μm","溫度(°C)","濕度(%)","正壓(Pa)","記錄者","結果"].map(h => (<th key={h} style={{ textAlign:"left", padding:"8px 10px", color:"#64748b", fontWeight:600, borderBottom:"1px solid rgba(255,255,255,0.06)", whiteSpace:"nowrap" }}>{h}</th>))}</tr></thead>
          <tbody>
            {sorted.map((r,i) => (
              <tr key={r.id} style={{ background:i%2===0?"rgba(255,255,255,0.02)":"transparent" }}>
                ...
                <td style={{ padding:"8px 10px", color:r.particles05>1000?"#ef4444":r.particles05>800?"#eab308":"#94a3b8", fontWeight:r.particles05>800?700:400 }}>{r.particles05.toLocaleString()}</td>
                <td style={{ padding:"8px 10px", color:"#94a3b8" }}>{r.particles1.toLocaleString()}</td>
                <td style={{ padding:"8px 10px", color:r.particles5>35?"#ef4444":r.particles5>20?"#eab308":"#94a3b8", fontWeight:r.particles5>20?700:400 }}>{r.particles5}</td>
                <td style={{ padding:"8px 10px", color:r.temp>23||r.temp<21?"#ef4444":"#94a3b8" }}>{r.temp.toFixed(1)}</td>
                <td style={{ padding:"8px 10px", color:r.humidity>50||r.humidity<40?"#ef4444":"#94a3b8" }}>{r.humidity.toFixed(1)}</td>
                <td style={{ padding:"8px 10px", color:r.pressure<10?"#ef4444":"#94a3b8" }}>{r.pressure.toFixed(1)}</td>
