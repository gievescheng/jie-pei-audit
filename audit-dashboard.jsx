...
function NonConformanceTab({ nonConformances: items, setNonConformances: setItems }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {enriched.map(item => (
          <div key={item.id} style={{ background: item.overdue?"rgba(239,68,68,0.12)":"rgba(255,255,255,0.03)", border: `1px solid ${item.overdue?"rgba(239,68,68,0.3)":"rgba(255,255,255,0.08)"}`, borderRadius: 12, padding: "14px 18px" }}>
...
function ProductionTab({ envRecords, prodRecords, setProdRecords, qualityRecords, setQualityRecords }) {
  ...
  const prodLowYield = prodRecords.filter(row => Number(row.yieldRate) < 95).length;
  const qualityNg = qualityRecords.filter(row => ["NG", "FAIL"].includes(String(row.result).toUpperCase())).length;
  const envWarning = envRecords.filter(row => row.result && row.result !== "合格").length;

  return (
    <div>
      <SectionHeader title="記錄匯出" color="#0ea5e9" />
      <div style={{ display:"flex", gap:12, marginBottom:20, flexWrap:"wrap" }}>
        <StatCard label="生產記錄" value={prodRecords.length} color="#38bdf8" sub={prodLowYield > 0 ? prodLowYield + " 批低良率" : "良率正常"} />
        <StatCard label="進料檢驗" value={qualityRecords.length} color="#22c55e" sub={qualityNg > 0 ? qualityNg + " 批異常" : "全部合格"} />
        <StatCard label="環境監測" value={envRecords.length} color="#14b8a6" sub={envWarning > 0 ? envWarning + " 筆警示" : "無警示"} />
      </div>

      <div style={{ background:"rgba(14,165,233,0.06)", border:"1px solid rgba(14,165,233,0.2)", borderRadius:14, padding:18, marginBottom:20 }}>
        <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
          <button onClick={() => downloadRecords("production", prodRecords, "生產記錄.xlsx")} disabled={downloadType !== ""} style={{ background:"linear-gradient(135deg,#0369a1,#0ea5e9)", border:"none", borderRadius:10, color:"#fff", cursor:"pointer", padding:"10px 16px", fontSize:13, fontWeight:700, opacity: downloadType && downloadType !== "production" ? 0.6 : 1 }}>{downloadType === "production" ? "下載中..." : "下載生產記錄"}</button>
          <button onClick={() => downloadRecords("quality", qualityRecords, "品質檢驗記錄.xlsx")} disabled={downloadType !== ""} style={{ background:"linear-gradient(135deg,#15803d,#22c55e)", border:"none", borderRadius:10, color:"#fff", cursor:"pointer", padding:"10px 16px", fontSize:13, fontWeight:700, opacity: downloadType && downloadType !== "quality" ? 0.6 : 1 }}>{downloadType === "quality" ? "下載中..." : "下載品質檢驗記錄"}</button>
          <button onClick={() => downloadRecords("env", envRecords, "環境監測記錄.xlsx")} disabled={downloadType !== ""} style={{ background:"linear-gradient(135deg,#0f766e,#14b8a6)", border:"none", borderRadius:10, color:"#fff", cursor:"pointer", padding:"10px 16px", fontSize:13, fontWeight:700, opacity: downloadType && downloadType !== "env" ? 0.6 : 1 }}>{downloadType === "env" ? "下載中..." : "下載環境監測記錄"}</button>
        </div>
        {message && <div style={{ marginTop:12, fontSize:12, color:"#bae6fd" }}>{message}</div>}
      </div>
      ...
      <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:14, padding:18, marginBottom:20 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12, gap:12, flexWrap:"wrap" }}>
          <div>
            <div style={{ fontSize:15, fontWeight:700, color:"#e2e8f0" }}>生產批次</div>
            <div style={{ fontSize:12, color:"#64748b", marginTop:4 }}>可編輯後直接輸出為生產記錄 Excel。</div>
          </div>
          <button onClick={addProdRecord} style={{ background:"rgba(56,189,248,0.16)", border:"1px solid rgba(56,189,248,0.25)", borderRadius:10, color:"#7dd3fc", cursor:"pointer", padding:"10px 14px", fontSize:13, fontWeight:700 }}>新增列</button>
        </div>
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", minWidth:980 }}>
            <thead><tr style={{ color:"#94a3b8", fontSize:12, textAlign:"left" }}>{["批號","客戶","產品","投入數","良品數","不良數","良率 %","不良原因","作業員","備註",""].map(head => <th key={head} style={{ padding:"10px 8px", borderBottom:"1px solid rgba(255,255,255,0.08)" }}>{head}</th>)}</tr></thead>
            <tbody>
              {prodRecords.map((row, index) => (
                <tr key={row.lot + "-" + index}>
                  <td style={{ padding:"8px" }}><input value={row.lot} onChange={e => updateProdRecord(index, "lot", e.target.value)} style={inputStyle} /></td>
                  <td style={{ padding:"8px" }}><input value={row.customer} onChange={e => updateProdRecord(index, "customer", e.target.value)} style={inputStyle} /></td>
                  <td style={{ padding:"8px" }}><input value={row.product} onChange={e => updateProdRecord(index, "product", e.target.value)} style={inputStyle} /></td>
                  <td style={{ padding:"8px" }}><input type="number" value={row.input} onChange={e => updateProdRecord(index, "input", e.target.value)} style={inputStyle} /></td>
                  <td style={{ padding:"8px" }}><input type="number" value={row.good} onChange={e => updateProdRecord(index, "good", e.target.value)} style={inputStyle} /></td>
                  <td style={{ padding:"8px" }}><input type="number" value={row.defect} onChange={e => updateProdRecord(index, "defect", e.target.value)} style={inputStyle} /></td>
                  <td style={{ padding:"8px", color:Number(row.yieldRate) < 95 ? "#fca5a5" : "#86efac", fontWeight:700 }}>{row.yieldRate === "" ? "--" : row.yieldRate}</td>
                  <td style={{ padding:"8px" }}><input value={Array.isArray(row.defectReasons) ? row.defectReasons.join(", ") : row.defectReasons} onChange={e => updateProdRecord(index, "defectReasons", e.target.value.split(/[;,]/).map(item => item.trim()).filter(Boolean))} style={inputStyle} /></td>
                  <td style={{ padding:"8px" }}><input value={row.operator} onChange={e => updateProdRecord(index, "operator", e.target.value)} style={inputStyle} /></td>
                  <td style={{ padding:"8px" }}><input value={row.note} onChange={e => updateProdRecord(index, "note", e.target.value)} style={inputStyle} /></td>
                  <td style={{ padding:"8px" }}><button onClick={() => setProdRecords(prev => prev.filter((_, i) => i !== index))} style={{ background:"rgba(239,68,68,0.14)", border:"1px solid rgba(239,68,68,0.25)", borderRadius:8, color:"#fca5a5", cursor:"pointer", padding:"8px 10px" }}>刪除</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:14, padding:18, marginBottom:20 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12, gap:12, flexWrap:"wrap" }}>
          <div>
            <div style={{ fontSize:15, fontWeight:700, color:"#e2e8f0" }}>進料檢驗</div>
            <div style={{ fontSize:12, color:"#64748b", marginTop:4 }}>原料 / 品質檢驗資料可直接輸出成 Excel。</div>
          </div>
          <button onClick={addQualityRecord} style={{ background:"rgba(34,197,94,0.14)", border:"1px solid rgba(34,197,94,0.25)", borderRadius:10, color:"#86efac", cursor:"pointer", padding:"10px 14px", fontSize:13, fontWeight:700 }}>新增列</button>
        </div>
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", minWidth:1080 }}>
            <thead><tr style={{ color:"#94a3b8", fontSize:12, textAlign:"left" }}>{["材料名稱","批號","數量","規格","檢驗數量","PH","比重","RI","旋光值","結果","備註",""].map(head => <th key={head} style={{ padding:"10px 8px", borderBottom:"1px solid rgba(255,255,255,0.08)" }}>{head}</th>)}</tr></thead>
            <tbody>
              {qualityRecords.map((row, index) => (
                <tr key={row.batchNo + "-" + index}>
                  {["materialName","batchNo","quantity","spec","inspQty","ph","density","ri","rotation","result","note"].map(field => (
                    <td key={field} style={{ padding:"8px" }}>
                      <input value={row[field]} onChange={e => updateQualityRecord(index, field, e.target.value)} style={inputStyle} />
                    </td>
                  ))}
                  <td style={{ padding:"8px" }}><button onClick={() => setQualityRecords(prev => prev.filter((_, i) => i !== index))} style={{ background:"rgba(239,68,68,0.14)", border:"1px solid rgba(239,68,68,0.25)", borderRadius:8, color:"#fca5a5", cursor:"pointer", padding:"8px 10px" }}>刪除</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ background:"rgba(124,58,237,0.08)", border:"1px solid rgba(124,58,237,0.2)", borderRadius:14, padding:18 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, flexWrap:"wrap", marginBottom:12 }}>
          <div>
            <div style={{ fontSize:15, fontWeight:700, color:"#e9d5ff" }}>出貨單草稿產生</div>
            <div style={{ fontSize:12, color:"#c4b5fd", marginTop:4 }}>依現有訂單 / LOT 資料產生 `14.3 出貨單` 草稿。</div>
          </div>
          <Badge color="#a78bfa">{shipmentLoading ? "載入中" : shipmentOrders.length + " 筆訂單"}</Badge>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(220px, 1fr))", gap:12 }}>
          ...
          <div><div style={{ fontSize:12, color:"#cbd5e1", marginBottom:6 }}>日期</div><input type="date" value={shipmentForm.date} onChange={e => setShipmentForm(prev => ({ ...prev, date:e.target.value }))} style={inputStyle} /></div>
          <div><div style={{ fontSize:12, color:"#cbd5e1", marginBottom:6 }}>部門</div><input value={shipmentForm.department} onChange={e => setShipmentForm(prev => ({ ...prev, department:e.target.value }))} style={inputStyle} /></div>
          <div><div style={{ fontSize:12, color:"#cbd5e1", marginBottom:6 }}>申請人</div><input value={shipmentForm.requester} onChange={e => setShipmentForm(prev => ({ ...prev, requester:e.target.value }))} style={inputStyle} /></div>
          <div><div style={{ fontSize:12, color:"#cbd5e1", marginBottom:6 }}>產品</div><input value={shipmentForm.product_name} onChange={e => setShipmentForm(prev => ({ ...prev, product_name:e.target.value }))} style={inputStyle} /></div>
          <div><div style={{ fontSize:12, color:"#cbd5e1", marginBottom:6 }}>規格</div><input value={shipmentForm.spec} onChange={e => setShipmentForm(prev => ({ ...prev, spec:e.target.value }))} style={inputStyle} /></div>
          <div><div style={{ fontSize:12, color:"#cbd5e1", marginBottom:6 }}>數量</div><input value={shipmentForm.quantity} onChange={e => setShipmentForm(prev => ({ ...prev, quantity:e.target.value }))} style={inputStyle} /></div>
          <div><div style={{ fontSize:12, color:"#cbd5e1", marginBottom:6 }}>單位</div><input value={shipmentForm.unit} onChange={e => setShipmentForm(prev => ({ ...prev, unit:e.target.value }))} style={inputStyle} /></div>
        </div>

        <div style={{ marginTop:12 }}>
          <div style={{ fontSize:12, color:"#cbd5e1", marginBottom:6 }}>訂單編號 / 批號顯示</div>
          <input value={shipmentForm.batch_display} onChange={e => setShipmentForm(prev => ({ ...prev, batch_display:e.target.value }))} style={inputStyle} />
        </div>
...
function AIWorkbenchTab({ documents, manuals, nonConformances }) {
  ...
  return (
    <div>
      <SectionHeader title="AI 工作台（V2）" count={sources.length} color="#38bdf8" />
      <div style={{ display:"flex", gap:12, marginBottom:20, flexWrap:"wrap" }}>
        <StatCard label="文件來源" value={sources.length} color="#38bdf8" sub="可直接送入文件稽核" />
        <StatCard label="資料庫模式" value={serviceInfo.database_mode || "unknown"} color={serviceInfo.database_mode === "postgresql" ? "#22c55e" : "#f59e0b"} sub={(serviceInfo.database_status && serviceInfo.database_status.using_fallback) ? "已退回 SQLite" : "MVP 可用"} />
      ...
      <div style={{ background:"rgba(56,189,248,0.08)", border:"1px solid rgba(56,189,248,0.18)", borderRadius:14, padding:18, marginBottom:18 }}>
        <div style={{ display:"grid", gridTemplateColumns:"1.2fr 0.8fr", gap:12, alignItems:"end" }}>
          ...
          <button onClick={runSearch} disabled={searchBusy} style={{ background:"linear-gradient(135deg,#0284c7,#38bdf8)", border:"none", borderRadius:10, color:"#fff", cursor:"pointer", padding:"10px 16px", fontSize:13, fontWeight:700, opacity: searchBusy ? 0.6 : 1 }}>{searchBusy ? "搜尋中..." : "測試文件搜尋"}</button>
        </div>
        ...
        <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginTop:12 }}>
          <button onClick={() => ingestPaths(sources.map(item => item.path))} disabled={ingestBusy || sources.length === 0} style={{ background:"linear-gradient(135deg,#0f766e,#14b8a6)", border:"none", borderRadius:10, color:"#fff", cursor:"pointer", padding:"10px 16px", fontSize:13, fontWeight:700, opacity:(ingestBusy || sources.length === 0) ? 0.6 : 1 }}>{ingestBusy ? "匯入中..." : "匯入全部 ISO 文件"}</button>
          <button onClick={() => ingestPaths(selectedPath ? [selectedPath] : [])} disabled={ingestBusy || !selectedPath} style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.14)", borderRadius:10, color:"#e2e8f0", cursor:"pointer", padding:"10px 16px", fontSize:13, fontWeight:700, opacity:(ingestBusy || !selectedPath) ? 0.6 : 1 }}>只匯入目前選中文件</button>
          <button onClick={() => clearRuntimeCache("all")} disabled={cacheBusy} style={{ background:"rgba(239,68,68,0.12)", border:"1px solid rgba(239,68,68,0.24)", borderRadius:10, color:"#fecaca", cursor:"pointer", padding:"10px 16px", fontSize:13, fontWeight:700, opacity: cacheBusy ? 0.6 : 1 }}>{cacheBusy ? "清理中..." : "清除全部快取"}</button>
          <button onClick={() => clearRuntimeCache("compare")} disabled={cacheBusy} style={{ background:"rgba(245,158,11,0.1)", border:"1px solid rgba(245,158,11,0.22)", borderRadius:10, color:"#fde68a", cursor:"pointer", padding:"10px 16px", fontSize:13, fontWeight:700, opacity: cacheBusy ? 0.6 : 1 }}>只清比對快取</button>
          <button onClick={() => clearRuntimeCache("audit")} disabled={cacheBusy} style={{ background:"rgba(59,130,246,0.1)", border:"1px solid rgba(59,130,246,0.22)", borderRadius:10, color:"#bfdbfe", cursor:"pointer", padding:"10px 16px", fontSize:13, fontWeight:700, opacity: cacheBusy ? 0.6 : 1 }}>只清稽核快取</button>
...
function DocumentsManagerTab({ documents, setDocuments, manuals }) {
  ...
  return (
    <div>
      <SectionHeader title="文件管理（ISO 架構）" count={documents.length + manuals.length} color="#a78bfa" />
      <div style={{ display:"flex", gap:12, marginBottom:20, flexWrap:"wrap" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
        <div style={{ background:"rgba(167,139,250,0.08)", border:"1px solid rgba(167,139,250,0.2)", borderRadius:14, padding:18 }}>
          <div style={{ fontSize:15, fontWeight:800, color:"#ddd6fe", marginBottom:12 }}>第一層：品質手冊與管理手冊</div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(280px, 1fr))", gap:12 }}>
            {iso.firstLevel.map(doc => <IsoDocumentCard key={doc.id} doc={doc} accent="#a78bfa" showPaths />)}
          </div>
        </div>

        <div style={{ background:"rgba(96,165,250,0.08)", border:"1px solid rgba(96,165,250,0.2)", borderRadius:14, padding:18 }}>
          <div style={{ fontSize:15, fontWeight:800, color:"#bfdbfe", marginBottom:12 }}>第二層：管理程序與其表單 / 記錄</div>
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ background:"rgba(20,184,166,0.08)", border:"1px solid rgba(20,184,166,0.2)", borderRadius:14, padding:18 }}>
          <div style={{ fontSize:15, fontWeight:800, color:"#99f6e4", marginBottom:12 }}>第三層：作業指導書</div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(280px, 1fr))", gap:12 }}>
            {iso.thirdLevel.map(doc => <IsoDocumentCard key={doc.id} doc={doc} accent="#14b8a6" showPaths />)}
          </div>
        </div>

        <details style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:14, padding:18 }}>
...
function LibraryHierarchyTab({ documents, manuals }) {
  ...
  return (
    <div>
      <SectionHeader title="文件庫（ISO 分層瀏覽）" count={documents.length + manuals.length} color="#f97316" />
      <div style={{ display:"flex", gap:10, marginBottom:18, flexWrap:"wrap" }}>
        ...
        {["全部", "一階文件", "二階文件", "三階文件"].map(item => (
          <button key={item} onClick={() => setLevelFilter(item)} style={{ background:levelFilter===item?"linear-gradient(135deg,#ea580c,#f97316)":"rgba(255,255,255,0.05)", border:"1px solid " + (levelFilter===item ? "#f97316" : "rgba(255,255,255,0.1)"), borderRadius:8, color:levelFilter===item?"#fff":"#94a3b8", cursor:"pointer", padding:"8px 14px", fontSize:12, fontWeight:700 }}>{item}</button>
        ))}
      </div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
        {filterByLevel("一階文件") && (
          <div style={{ background:"rgba(167,139,250,0.08)", border:"1px solid rgba(167,139,250,0.2)", borderRadius:14, padding:18 }}>
            <div style={{ fontSize:15, fontWeight:800, color:"#ddd6fe", marginBottom:12 }}>第一層文件</div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(280px, 1fr))", gap:12 }}>
        ...
        {filterByLevel("二階文件") && (
          <div style={{ background:"rgba(96,165,250,0.08)", border:"1px solid rgba(96,165,250,0.2)", borderRadius:14, padding:18 }}>
            <div style={{ fontSize:15, fontWeight:800, color:"#bfdbfe", marginBottom:12 }}>第二層文件與其表單 / 記錄</div>
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        {filterByLevel("三階文件") && (
          <div style={{ background:"rgba(20,184,166,0.08)", border:"1px solid rgba(20,184,166,0.2)", borderRadius:14, padding:18 }}>
            <div style={{ fontSize:15, fontWeight:800, color:"#99f6e4", marginBottom:12 }}>第三層文件</div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(280px, 1fr))", gap:12 }}>
...
export default function App() {
  ...
  function renderTab() {
    switch(activeTab) {
      ...
      case "production":     return <ProductionTab envRecords={envRecords} prodRecords={prodRecords} setProdRecords={setProdRecords} qualityRecords={qualityRecords} setQualityRecords={setQualityRecords} />;
