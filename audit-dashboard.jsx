...
function KpiDashboard({ instruments, suppliers, nonConformances, auditPlans, envRecords, prodRecords, qualityRecords }) {
  ...
  const envPass  = envs.filter(r => r.result === "合格").length;
  const envPassRate = envTotal > 0 ? Math.round(envPass / envTotal * 100) : null;
  const envByLoc = {};
  envs.forEach(r => {
    const loc = r.location || r.point || "未知";
    if (!envByLoc[loc]) envByLoc[loc] = { pass: 0, total: 0 };
    envByLoc[loc].total++;
    if (r.result === "合格") envByLoc[loc].pass++;
  });
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:16 }}>
        {panel(<>
          {sectionTitle("各監測點環境合格率（%）")}
          {envLocData.length === 0
            ? <div style={{ color:"#475569", fontSize:12 }}>尚無環境記錄</div>
