...
def generate_http(base_url: str, prompt: str, model: Optional[str] = None, api_key: Optional[str] = None,
    ...
    if chat:
        ...
        payload = {"prompt": prompt, "temperature": temperature, "max_tokens": max_tokens, "stream": False}
        if model:
            payload["model"] = model
    url = f"{base}{endpoint}"
...
def main(argv=None):
    ...
    conn = LocalModelConnector(mode=args.mode, base_url=args.base_url, api_key=args.api_key,
                               model=args.model, chat=args.chat, device=args.device)
    try:
        out = conn.generate(args.prompt, temperature=args.temperature,
